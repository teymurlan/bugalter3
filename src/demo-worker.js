export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/health') {
      return json({ ok: true, mode: 'demo-bot', hasBotToken: Boolean(env.TELEGRAM_BOT_TOKEN), hasWebhookSecret: Boolean(env.TELEGRAM_WEBHOOK_SECRET), hasAdminIds: adminIds(env).length > 0 });
    }

    if (url.pathname === '/telegram/status' && request.method === 'GET') {
      const base = { ok: true, hasBotToken: Boolean(env.TELEGRAM_BOT_TOKEN), hasWebhookSecret: Boolean(env.TELEGRAM_WEBHOOK_SECRET), hasAdminIds: adminIds(env).length > 0, expectedWebhook: `${url.origin}/telegram/webhook` };
      if (!env.TELEGRAM_BOT_TOKEN) return json(base);
      try {
        const [me, webhook] = await Promise.all([telegram(env.TELEGRAM_BOT_TOKEN, 'getMe', {}), telegram(env.TELEGRAM_BOT_TOKEN, 'getWebhookInfo', {})]);
        return json({ ...base, bot: { id: me.id, username: me.username, first_name: me.first_name }, webhook: { url: webhook.url || '', pending_update_count: webhook.pending_update_count || 0, last_error_date: webhook.last_error_date || null, last_error_message: webhook.last_error_message || '', max_connections: webhook.max_connections || null } });
      } catch (error) { return json({ ...base, ok: false, error: error.message || 'Telegram API error' }, 500); }
    }
    ...
    if (url.pathname === '/api/demo-order' && request.method === 'POST') {
      ...
      if (!order.order_number || !order.customer_name || !order.date || !order.address) return json({ ok: false, error: 'Недостаточно данных заявки' }, 400);
      const ids = adminIds(env);
      if (!ids.length) return json({ ok: true, notified: 0, warning: 'ADMIN_TELEGRAM_IDS is not configured' });
      const text = `<b>НОВАЯ ЗАЯВКА HOUSE CLEANING</b>\n\n<b>${html(order.order_number)}</b>\nКлиент: <b>${html(order.customer_name)}</b>\nТелефон: ${html(order.phone || '—')}\nУборка: ${html(order.service_name || '—')}\nПлощадь: <b>${Number(order.area || 0)} м²</b>\nДата: <b>${html(order.date)} · ${html(order.time || '—')}</b>\nАдрес: ${html(`${order.city || ''}, ${order.address || ''}`)}\nДополнительно: ${html((order.addon_names || []).join(', ') || 'нет')}\nФото: ${Number(order.photo_count || 0)}\n\nTelegram: ${telegramUser.username ? '@' + html(telegramUser.username) : 'ID ' + telegramUser.id}`;
      const adminUrl = `${url.origin}/?demo=1&admin=1`;
      const markup = { inline_keyboard: [[{ text: 'Открыть панель заказов', web_app: { url: adminUrl }, style: 'primary' }], [{ text: 'Написать клиенту', url: `tg://user?id=${telegramUser.id}` }]] };
      const results = await Promise.allSettled(ids.map((id) => telegram(env.TELEGRAM_BOT_TOKEN, 'sendMessage', { chat_id: id, text, parse_mode: 'HTML', reply_markup: markup })));
    ...
    if (url.pathname === '/telegram/webhook' && request.method === 'POST') {
      ...
      if (text === '/myid' || text === '/id') {
        await safeTelegram(env, 'sendMessage', { chat_id: chatId, text: `Ваш Telegram ID: ${chatId}\n\nДобавьте это число в Cloudflare как ADMIN_TELEGRAM_IDS, чтобы получать уведомления о новых заказах.` });
        return new Response('OK');
      }

      if (text === '/admin' && isAdmin(env, chatId)) {
        await safeTelegram(env, 'sendMessage', { chat_id: chatId, text: '<b>Панель HOUSE CLEANING</b>\n\nОткройте список заказов и фильтры.', parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: 'Открыть админ-панель', web_app: { url: `${url.origin}/?demo=1&admin=1` }, style: 'primary' }]] } });
        return new Response('OK');
      }
      ...
      if (text.startsWith('/start') || text === '/help') {
        ...
        const greeting = `<b>HOUSE CLEANING</b>\n\nЗдравствуйте, ${html(firstName)}.\n\nЗаказать уборку теперь можно прямо здесь:\n• выберите вид уборки\n• укажите площадь и адрес\n• добавьте фотографии\n• выберите свободную дату и время\n\nОформление занимает около 2 минут.`;
        await safeTelegram(env, 'sendMessage', { chat_id: chatId, text: greeting, parse_mode: 'HTML', reply_markup: { inline_keyboard: keyboard } });
      }
      return new Response('OK');
    }
    ...
    if (url.pathname === '/telegram/setup' && request.method === 'POST') {
      if (!env.TELEGRAM_BOT_TOKEN || !env.TELEGRAM_WEBHOOK_SECRET) return json({ ok: false, error: 'Сначала добавьте TELEGRAM_BOT_TOKEN и TELEGRAM_WEBHOOK_SECRET в Cloudflare' }, 400);
      let body = {};
      try { body = await request.json(); } catch { return json({ ok: false, error: 'Некорректный запрос' }, 400); }
      if (body.secret !== env.TELEGRAM_WEBHOOK_SECRET) return json({ ok: false, error: 'Неверный секрет' }, 403);
      const webhookUrl = `${url.origin}/telegram/webhook`;
      try {
        const result = await telegram(env.TELEGRAM_BOT_TOKEN, 'setWebhook', { url: webhookUrl, secret_token: env.TELEGRAM_WEBHOOK_SECRET, allowed_updates: ['message', 'edited_message'], drop_pending_updates: false });
        const webhook = await telegram(env.TELEGRAM_BOT_TOKEN, 'getWebhookInfo', {});
        return json({ ok: true, webhookUrl, telegram: result, webhook });
      } catch (error) { return json({ ok: false, error: error.message || 'Не удалось установить webhook' }, 500); }
    }
...
function adminIds(env) { return String(env.ADMIN_TELEGRAM_IDS || '').split(',').map((v) => v.trim()).filter((v) => /^-?\d+$/.test(v)); }
function isAdmin(env, id) { return adminIds(env).includes(String(id)); }
function html(value) { return String(value ?? '').replace(/[&<>"']/g, (char) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[char] || char)); }
