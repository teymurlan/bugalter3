export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/health') {
      return json({ ok: true, mode: 'demo-bot-v4', hasBotToken: !!env.TELEGRAM_BOT_TOKEN, hasWebhookSecret: !!env.TELEGRAM_WEBHOOK_SECRET, adminCount: admins(env).length });
    }
    ...
    if (url.pathname === '/api/demo-order' && request.method === 'POST') {
      ...
      const adminKeyboard = event === 'created' ? { inline_keyboard: [
        [{ text: 'Подтвердить', callback_data: cb('c', user.id, order.order_number) }, { text: 'Отменить', callback_data: cb('x', user.id, order.order_number) }],
        [{ text: 'Написать клиенту', url: `tg://user?id=${user.id}` }],
        [{ text: 'Панель заказов', web_app: { url: `${url.origin}/?demo=1&admin=1` } }],
      ] } : { inline_keyboard: [[{ text: 'Написать клиенту', url: `tg://user?id=${user.id}` }]] };

      const results = await Promise.allSettled(admins(env).map(id => tg(env, 'sendMessage', { chat_id: id, text: adminText, parse_mode: 'HTML', reply_markup: adminKeyboard })));
      ...
      const clientText = event === 'created'
        ? `<b>Заявка получена</b>\n\n<b>${esc(order.order_number)}</b>\n${esc(order.service_name)} · ${order.area} м²\n${esc(order.date)} · ${esc(order.time || '—')}\n${esc(`${order.city}, ${order.address}`)}\n\nАдминистратор получил заявку. После подтверждения бот пришлёт отдельное сообщение.`
        : `<b>Заявка отменена</b>\n\n<b>${esc(order.order_number)}</b>\nОтмена принята. Администратор уведомлён.`;
      const clientSent = await safe(env, 'sendMessage', { chat_id: user.id, text: clientText, parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: 'Открыть HOUSE CLEANING', web_app: { url: `${url.origin}/?demo=1` } }]] } });
      return json({ ok: true, event, adminConfigured: admins(env).length > 0, adminNotified, adminErrors, clientNotified: !!clientSent });
    }
    ...
    if (url.pathname === '/telegram/webhook' && request.method === 'POST') {
      ...
      if (text === '/notifytest') {
        ...
        const ids = admins(env); const rs = await Promise.allSettled(ids.map(id => tg(env, 'sendMessage', { chat_id: id, text: '<b>Тест HOUSE CLEANING</b>\nУведомления администратора работают.', parse_mode: 'HTML' })));
      ...
      if (text === '/admin' && isAdmin(env, chatId)) {
        await safe(env, 'sendMessage', { chat_id: chatId, text: '<b>HOUSE CLEANING · Администратор</b>', parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: 'Открыть заказы', web_app: { url: `${url.origin}/?demo=1&admin=1` } }]] } });
        return new Response('OK');
      }
...
async function handleCallback(q, env, origin) {
  ...
  if(q.message?.chat?.id&&q.message?.message_id){
    const keys=action==='c'?{inline_keyboard:[[{text:'Завершить уборку',callback_data:cb('d',clientId,number)}],[{text:'Отменить',callback_data:cb('x',clientId,number)}],[{text:'Написать клиенту',url:`tg://user?id=${clientId}`}]]}:{inline_keyboard:[[{text:'Написать клиенту',url:`tg://user?id=${clientId}`}]]};
...
async function sendStatus(env, clientId, number, status, origin){
  ...
  if(!text)return null;
  return safe(env,'sendMessage',{chat_id:clientId,text,parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:'Открыть HOUSE CLEANING',web_app:{url:`${origin}/?demo=1`}}]]}});
}
...
function admins(env){return String(env.ADMIN_TELEGRAM_IDS||'').split(',').map(v=>v.trim()).filter(v=>/^-?\d+$/.test(v))}
...
function setupPage(){return `<!doctype html><html lang="ru"><meta name="viewport" content="width=device-width"><style>body{font-family:-apple-system;background:#05090c;color:#fff;max-width:560px;margin:auto;padding:28px 18px}.c{background:#101820;border:1px solid #283542;border-radius:20px;padding:18px}input,button{width:100%;box-sizing:border-box;padding:14px;border-radius:14px;font-size:16px;margin-top:12px}input{background:#071016;color:#fff;border:1px solid #33404a}button{border:0;background:#f7bb38;font-weight:800}pre{white-space:pre-wrap;background:#071016;padding:12px;border-radius:12px}</style><h1>Telegram Setup</h1><div class="c"><button id="check">Проверить состояние</button><pre id="s"></pre><input id="secret" type="password" placeholder="TELEGRAM_WEBHOOK_SECRET"><button id="go">Установить webhook</button><pre id="o"></pre></div><script>check.onclick=async()=>s.textContent=JSON.stringify(await(await fetch('/telegram/status')).json(),null,2);go.onclick=async()=>o.textContent=JSON.stringify(await(await fetch('/telegram/setup',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({secret:secret.value})})).json(),null,2)</script>`}
