import baseWorker, { ConsentStore, AppStore } from './demo-worker-v20-mobile-fixes.js';
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/demo-admin-message' && request.method === 'POST') {
      ...
      const sent = await safeTelegram(env, 'sendMessage', {
        chat_id: clientId,
        text: templateText(template, order),
        parse_mode: 'HTML',
        ...(managerId ? { reply_markup: { inline_keyboard: [[{ text: 'Написать менеджеру', url: `tg://user?id=${managerId}` }]] } } : {}),
      });
...
function templateText(template, order) {
  ...
  return (map[template] || map.need_details).join('\n');
}
...
function formatDate(value) {
  ...
  return m ? `${m[3]}.${m[2]}.${m[1]}` : escapeHtml(raw || '—');
}
function formatTime(value) {
  const raw = String(value || '').trim();
  const m = raw.match(/^(\d{1,2}):(\d{2})/);
  return m ? `${String(Number(m[1])).padStart(2, '0')}:${m[2]}` : escapeHtml(raw || '—');
}
