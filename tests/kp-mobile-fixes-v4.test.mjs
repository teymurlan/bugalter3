...
assert.ok(wrangler.includes('src/demo-worker-v40-admin-diagnostic.js'), 'wrangler must deploy the current diagnostic wrapper');
assert.ok(diagnosticWorker.includes("./demo-worker-v39-admin-menu.js"), 'diagnostic wrapper must preserve the admin-menu wrapper');
assert.ok(menuWorker.includes("./demo-worker-v38-staff.js"), 'menu wrapper must preserve the current release worker');
