...
test('shared booking worker remains active with production chain intact', () => {
...
test('all devices use shared server availability', () => {
...
test('release 52 uses new visit renderer and fresh cache keys', () => {
  assert.match(index, /house-cleaning-release" content="52/);
  assert.match(index, /visit-type-v52\.css\?v=52/);
  assert.match(index, /app-release-v3\.js\?v=52/);
  assert.match(specialEntry, /app-release-v2\.js\?v=52/);
  assert.match(entry, /api-release-v2\.js\?v=52/);
  assert.match(entry, /state-release-v2\.js\?v=52/);
  assert.match(entry, /app-release\.js\?v=52/);
