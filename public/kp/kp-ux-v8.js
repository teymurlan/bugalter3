(() => {
  ...
  async function redrawTopHeader(page, quote) {
    ...
    // Полностью очищаем старую область логотипа слева.
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(M - 12, 42, 520, 210);

    // Полностью очищаем строку старого заголовка и служебных данных под ним.
    // Это гарантированно убирает остаточную надпись от прежнего логотипа.
    ...
    // Более аккуратный заголовок: только первая буква заглавная.
    const titleY = 298;
    ctx.textAlign = 'left';
    ctx.fillStyle = '#111';
