...
const FAQ = [
  ['Как рассчитывается стоимость?', 'Предварительная стоимость рассчитывается по площади, виду уборки и выбранным дополнительным услугам. Точную сумму менеджер подтверждает после оценки объекта и фотографий.'],
  ['Зачем нужны фотографии объекта?', 'Фотографии нужны до уборки для оценки состояния помещения и объёма работ.'],
  ...
  ['Можно ли отменить уборку?', 'Самостоятельная отмена доступна не позднее чем за 24 часа до начала уборки. Если осталось меньше 24 часов, свяжитесь с менеджером.'],
  ...
  ['Как работает программа лояльности?', 'После 3 успешно завершённых уборок действует скидка 5%, после 10 — 10%. Учитываются только завершённые уборки.'],
  ['Как работает приглашение друзей?', 'Друг получает 15% на первую успешную уборку по вашей ссылке. После её завершения вы получаете 15% на следующую уборку.'],
  ['Как оформить абонемент?', 'Выберите абонемент на 5 или 10 уборок и свяжитесь с менеджером. График и условия согласуются индивидуально.'],
  ['Как связаться с менеджером?', 'Можно написать менеджеру в Telegram или позвонить по номеру +7 999 210-79-77.'],
];
...
async function patchProfile(payload) {
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok || data?.ok === false) throw new Error(data?.error || 'Не удалось сохранить данные');
...
function subpageHeader(title, subtitle = '') {
  return `<button class="cc-back hc-fixed-back" type="button" data-back>← Назад</button><header class="cc-subpage-head"><span class="cc-kicker">HOUSE CLEANING</span><h1>${escapeHtml(title)}</h1>${subtitle ? `<p class="page-subtitle">${escapeHtml(subtitle)}</p>` : ''}</header>`;
}

function loyaltyLoadingCard() {
  return `<section class="card cc-loyalty-card"><div class="cc-loyalty-top"><span class="cc-menu-icon">${icon('shield')}</span><div><small>Ваш уровень</small><strong>Программа лояльности</strong></div><b>…</b></div><div class="cc-loyalty-progress"><i style="width:0%"></i></div><div class="cc-loyalty-bottom"><span>Проверяем историю уборок…</span></div></section>`;
}

function loyaltyCard(data = {}) {
  const completed = Number(data.completed_orders || 0);
  const discount = Number(data.loyalty_percent || 0);
  const target = completed < 3 ? 3 : completed < 10 ? 10 : 10;
  ...
  return `<section class="card cc-loyalty-card"><div class="cc-loyalty-top"><span class="cc-menu-icon">${icon('shield')}</span><div><small>Ваш уровень</small><strong>${discount ? `Скидка ${discount}%` : 'Программа лояльности'}</strong></div><b>${completed}</b></div><div class="cc-loyalty-progress"><i style="width:${progress}%"></i></div><div class="cc-loyalty-bottom"><span>${escapeHtml(note)}</span><small>После 3 уборок — 5%, после 10 — 10%</small></div></section>`;
}
...
export function renderConciergeProfile(root, navigate, params = {}) {
  ...
  root.innerHTML = `
    <div class="cc-profile-page">
      <header class="cc-profile-head"><div><span class="cc-kicker">HOUSE CLEANING</span><h1 class="page-title">Профиль</h1><p class="page-subtitle">Данные, скидки и настройки вашего сервиса.</p></div></header>
      ...
      <div class="card cc-menu">
        ...
        ${menuRow('faq', 'Частые вопросы', 'Оплата, отмена и сервис', 'faq')}
        ${menuRow('help', 'Связаться с менеджером', 'Telegram или телефон', 'chat')}
  ...
  getJson('/api/client-benefits').then((data) => {
    const slot = root.querySelector('[data-loyalty-slot]');
    if (slot && root.querySelector('.cc-profile-page')) slot.innerHTML = loyaltyCard(data);
  }).catch(() => {});
}
...
async function showReferral(root, navigate) {
  root.innerHTML = `${subpageHeader('Пригласить друзей', 'Друг получает 15% на первую уборку, вы — 15% после её успешного завершения.')}
    <section class="card cc-referral-hero"><span class="cc-referral-badge">15% + 15%</span><h2>Приглашайте по персональной ссылке</h2><p>Никаких кодов: просто отправьте ссылку другу.</p></section>
    <section class="card cc-ref-v2-stats" data-ref-stats><div><strong>…</strong><span>Приглашено</span></div><div><strong>…</strong><span>Оформили</span></div><div><strong>…</strong><span>Завершили</span></div><div><strong>…</strong><span>Доступно 15%</span></div></section>
    <div class="cc-ref-actions"><button class="primary-btn" type="button" data-share disabled>Поделиться приглашением</button><button class="secondary-btn" type="button" data-copy disabled>Скопировать ссылку</button></div>
    <div class="cc-policy-note"><strong>Награда учитывается только после успешно завершённой уборки друга.</strong> Отменённая заявка не сжигает его скидку.</div>`;
...
function showSubscriptions(root, navigate) {
  const plan = (count) => `<section class="card cc-pass-card"><div class="cc-pass-top"><span>Абонемент</span><strong>${count} уборок</strong></div><ul><li>Не нужно каждый раз оформлять заказ заново</li><li>Удобное планирование и согласованный график</li><li>Регулярный сервис</li><li>Персональное сопровождение менеджера</li></ul><button class="primary-btn" type="button" data-pass-manager>Оформить через менеджера</button></section>`;
  root.innerHTML = `${subpageHeader('Абонементы', 'Для регулярной уборки без лишних повторных оформлений.')}<div class="cc-pass-list">${plan(5)}${plan(10)}</div>`;
  root.querySelector('[data-back]').onclick = () => renderConciergeProfile(root, navigate);
  root.querySelectorAll('[data-pass-manager]').forEach((button) => button.onclick = () => showContact(root, navigate));
}

function showFaq(root, navigate) {
  root.innerHTML = `${subpageHeader('Частые вопросы', 'Короткие ответы на самые важные вопросы.')}
    <div class="cc-faq-list">${FAQ.map((item, index) => `<button class="cc-faq ${index === 0 ? 'open' : ''}" type="button" data-faq="${index}"><span class="cc-faq-q"><strong>${escapeHtml(item[0])}</strong><i>+</i></span><span class="cc-faq-a">${escapeHtml(item[1])}</span></button>`).join('')}</div>
    <section class="card hc-contact-panel"><div><small>Нужна помощь?</small><strong>Свяжитесь с менеджером</strong></div><div class="hc-contact-actions"><button class="hc-btn hc-btn-blue" type="button" data-contact-chat>Написать</button><button class="hc-btn hc-btn-green" type="button" data-contact-call>Позвонить</button></div></section>`;
  root.querySelector('[data-back]').onclick = () => renderConciergeProfile(root, navigate);
  root.querySelectorAll('[data-faq]').forEach((item) => item.onclick = () => {
    const wasOpen = item.classList.contains('open');
    root.querySelectorAll('[data-faq]').forEach((node) => node.classList.remove('open'));
    if (!wasOpen) item.classList.add('open');
  });
  root.querySelector('[data-contact-chat]').onclick = openManager;
  root.querySelector('[data-contact-call]').onclick = callManager;
}

function showContact(root, navigate) {
  root.innerHTML = `${subpageHeader('Связаться с менеджером', 'Выберите удобный способ связи.')}
    <section class="card hc-contact-page"><span class="cc-menu-icon">${icon('chat')}</span><div><h2>Мы на связи</h2><p>Напишите в Telegram или позвоните напрямую.</p></div><button class="hc-btn hc-btn-blue" type="button" data-chat>Написать менеджеру</button><button class="hc-btn hc-btn-green" type="button" data-call>Позвонить · +7 999 210-79-77</button></section>`;
  root.querySelector('[data-back]').onclick = () => renderConciergeProfile(root, navigate);
  root.querySelector('[data-chat]').onclick = openManager;
...
function showEdit(root, navigate) {
  const user = state.bootstrap?.user || {};
  const region = user.region === 'lo' ? 'lo' : 'spb';
  root.innerHTML = `${subpageHeader('Мои данные', 'Сохранённые данные можно быстро подставить при следующем заказе.')}
    <div class="card cc-form-card hc-profile-form">
      ${field('Имя', 'name', user.name || user.first_name || '')}
      ${field('Основной телефон', 'phone', user.phone || '', 'tel')}
      ${field('Дополнительный телефон', 'phone2', user.phone2 || '', 'tel', 'Необязательно')}
      <div class="field-label">Регион</div>
      <div class="hc-location-choice"><button class="hc-location-option ${region === 'spb' ? 'selected' : ''}" type="button" data-region="spb">Санкт-Петербург</button><button class="hc-location-option ${region === 'lo' ? 'selected' : ''}" type="button" data-region="lo">Ленинградская область</button></div>
      ${field('Город / населённый пункт', 'locality', user.locality || (region === 'spb' ? 'Санкт-Петербург' : ''))}
      ${field('Улица', 'street', user.street || '')}
      ${field('Дом', 'house', user.house || '')}
      <div class="hc-form-grid">${field('Квартира / офис', 'apartment', user.apartment || '', 'text', 'Необязательно')}${field('Этаж', 'floor', user.floor || '', 'text', 'Необязательно')}${field('Парадная / подъезд', 'entrance', user.entrance || '', 'text', 'Необязательно')}</div>
    </div>
    <button class="primary-btn" style="margin-top:14px" type="button" data-save>Сохранить данные</button>`;
  root.querySelector('[data-back]').onclick = () => renderConciergeProfile(root, navigate);
  let selectedRegion = region;
  root.querySelectorAll('[data-region]').forEach((button) => button.onclick = () => {
    selectedRegion = button.dataset.region;
    root.querySelectorAll('[data-region]').forEach((node) => node.classList.toggle('selected', node.dataset.region === selectedRegion));
    const locality = root.querySelector('[data-profile-field="locality"]');
    if (selectedRegion === 'spb' && locality && !locality.value.trim()) locality.value = 'Санкт-Петербург';
  });
  root.querySelector('[data-save]').onclick = async () => {
    const payload = { region: selectedRegion };
    ...
    if (hasAddress && (!payload.locality || !payload.street || !payload.house)) return showToast('Для сохранения адреса укажите город, улицу и дом', true);
...
function field(label, name, value, type = 'text', hint = '') {
  return `<div class="field"><label>${escapeHtml(label)}</label><input class="input" type="${type}" data-profile-field="${name}" value="${escapeHtml(value)}" placeholder="${escapeHtml(hint)}" data-label="${escapeHtml(label)}"></div>`;
}

function showRules(root, navigate) {
  root.innerHTML = `${subpageHeader('Правила и условия')}
    <div class="card cc-form-card hc-rules"><p>Мы работаем в Санкт-Петербурге и Ленинградской области.</p><p>Заявка считается подтверждённой после подтверждения менеджером HOUSE CLEANING.</p><p>Предварительная стоимость рассчитывается по площади и выбранным услугам. Точная стоимость может быть уточнена после оценки фотографий и параметров объекта.</p><p>Самостоятельная отмена доступна не позднее чем за 24 часа до начала уборки. Позже изменения и отмена согласовываются с менеджером.</p></div>`;
  root.querySelector('[data-back]').onclick = () => renderConciergeProfile(root, navigate);
}
