...
import { renderBooking } from './views/booking-v5.js?v=63';
import { renderConciergeHome } from './views/concierge-home-v4.js?v=63';
import { renderConciergeOrders } from './views/concierge-orders-v4.js?v=63';
import { renderConciergeProfile } from './views/concierge-profile-v4.js?v=63';
...
function configureFocusContext() {
  document.addEventListener('focusin', (event) => {
    ...
    const label = target.dataset.label || target.closest('.field')?.querySelector('label')?.textContent?.trim() || target.getAttribute('aria-label') || '';
    if (!label) return;
...
function moveIndicator(button, retry = 0) {
  ...
  requestAnimationFrame(() => {
    ...
    if ((!navRect.width || !buttonRect.width) && retry < 4) { setTimeout(() => moveIndicator(button, retry + 1), 40); return; }
...
function haptic() { try { tg?.HapticFeedback?.selectionChanged?.(); } catch {} }
...
async function hydrateClientProfile() {
  ...
  } catch (error) { console.warn('Profile hydrate skipped', error); }
}
